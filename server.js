...
function buildLocalNanoLink(req, id) {
  return `${req.protocol}://${req.get('host')}/n/${id}`
}

function formatEntry(row, req, format = 'card') {
  let favicon = ''
  let barcodeLink = ((config.has('useTecITBarcodeGenerator') && config.useTecITBarcodeGenerator) || (req.query.barcode && req.query.barcode == 'yes')) && row.nanoid && row.nanoid.length == 8 ?
    `<br><img alt='Barcode for ${ row.nanoid } by Barcode Generator TEC-IT'
       src='https://barcode.tec-it.com/barcode.ashx?data=${buildLocalNanoLink(req, row.nanoid)}&code=DataMatrix&multiplebarcodes=false&translate-esc=false&unit=Min&dpi=96&imagetype=Png&rotation=0&color=%23000000&bgcolor=%23ffffff&codepage=Default&qunit=Mm&quiet=0&eclevel=L&dmsize=Default'/>`
    :
    ''
  ...
  switch (format) {
    case 'card':
      ...
      return `
        ...
        <div class='col-md-8'>
        <div class='card-body'>
          <p class='card-text small'><a href='/n/${row.nanoid}' target='_blank' title='${row.title ? row.title : row.url}'>${row.title ? row.title : row.nanoid}</a>${row.toread && row.toread.length == 13 ? `&#128065` : ''}${barcodeLink}
          </p>
          </div>
        </div>
        </div>
      ...
      [
        ...
        if (rows && rows.length) {
          ...
          rows.forEach((row) => {
            ...
            res.write(formatEntry(row, req, listFormat))
        ...
        if (config.has('useTecITBarcodeGenerator') && config.useTecITBarcodeGenerator) {
          res.write(`<em>Barcodes generated by <a href='https://www.tec-it.com' title='Barcode Software by TEC-IT' target='_blank'>TEC-IT Barcode Generator</em>`)
        }
        res.write(endHtml())
        res.end()
    })
